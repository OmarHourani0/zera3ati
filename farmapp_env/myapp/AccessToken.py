...
class AccessToken:
    ...
    def build(self):
        ...
        val = (self.appID if isinstance(self.appID, str) else self.appID.decode('utf-8')) + \
            (self.channelName if isinstance(self.channelName, str) else self.channelName.decode('utf-8')) + \
            (self.uidStr if isinstance(self.uidStr, str) else self.uidStr.decode('utf-8')) + \
            m.hex()  # Convert bytes to a hexadecimal string
