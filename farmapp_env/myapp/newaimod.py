...
class MarketPrice:
    ...
    def read_output(self, stream, output_list):
        while True:
    ...
    def get_prices(self, language, crops_list=None, specified_month=None):
        ...
        # Read output in a loop using threads to prevent deadlocks
        stdout_lines = []
        stderr_lines = []
        stdout_thread = threading.Thread(target=self.read_output, args=(self.process.stdout, stdout_lines))
        stderr_thread = threading.Thread(target=self.read_output, args=(self.process.stderr, stderr_lines))
        stdout_thread.start()
        stderr_thread.start()
        stdout_thread.join()
        stderr_thread.join()
        ...
        # Check for errors
        if stderr_lines:
            raise Exception(f"Error from subprocess: {' '.join(stderr_lines)}")

        output = "\n".join(stdout_lines)

        # Check if output is empty
        if not output.strip():
            raise Exception("Received empty output from subprocess")
